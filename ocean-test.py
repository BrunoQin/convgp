...
class DataPlaceholders(object):
    def __init__(self):
        self.ximage_flat = tf.placeholder(tf.float32, shape=[None, 150*160])
        self.x_image_reshaped = tf.reshape(self.ximage_flat, [-1, 150, 160, 1], name="img_reshaped")
...
def make_small_ocean_nn(x_placeholder, end_h=50):
    image_size = 150
...
def main():
    ...
    with tf_graph.as_default():
        ...
        nn_base = tf.make_template("sconvnet_kernel", make_small_ocean_nn, end_h=num_h)  # end h is the number of hidden
        # units at the end
    ...
    with gpf.defer_build():
        ...
        gp_model = gpf.models.SVGP(h, phs.label, kernel, likelihood, np.ones((num_inducing, num_h), gpf.settings.np_float),
                                   whiten=False, minibatch_size=None, num_data=x_train.shape[0])
